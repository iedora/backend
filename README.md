...
| `framework/Outbox/Framework.Outbox` | Reusable, independently-tested transactional-outbox library (entity, dispatcher, `IOutboxHandler`) — DbContext-agnostic, provider-agnostic, no broker. Packable; the first of the shared `Framework.*` libraries (each lives under `framework/<name>/` with its tests). |
...
- **Transactional outbox** — the reusable [`Framework.Outbox`](framework/Outbox/Framework.Outbox) library (its own [test suite](framework/Outbox/Framework.Outbox.Tests) on Testcontainers Postgres): `db.EnqueueOutbox(...)` stages a message in the same `SaveChangesAsync` as the request, and a background dispatcher routes it to an [`IOutboxHandler`](framework/Outbox/Framework.Outbox/IOutboxHandler.cs) with retry/backoff — so a crash after commit can't drop the effect. No message broker. Auth registers it with `AddOutbox<AuthDbContext>()` and a [`PasswordResetEmailHandler`](src/Iedora.Auth/Features/ForgotPassword/PasswordResetEmailHandler.cs) that sends via MailKit SMTP. Configure `Smtp:*` + `PasswordReset:ResetUrlBase` to deliver; single-instance dispatcher (add `FOR UPDATE SKIP LOCKED` for multiple replicas).
