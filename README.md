...
- **Result pattern** — expected failures are [`ErrorOr`](https://github.com/amantinband/error-or) values from an [error catalog](src/Iedora.Auth/Common/AuthErrors.cs) (no exceptions/null), mapped to RFC 9457 `ProblemDetails` by the reusable [`Framework.Web.ProblemResults`](framework/Web/Framework.Web/ProblemResults.cs) — the error `code` rides in the problem body as a machine-readable discriminator.
