...
| `src/Iedora.Worker` | The single, generic app-wide background worker. Composes each `*.Messaging` module and runs its outbox dispatcher (one per `DbContext`); scales to N replicas via `FOR UPDATE SKIP LOCKED`. |
...
- **Result pattern** — expected failures are [`ErrorOr`](https://github.com/amantinband/error-or) values from an [error catalog](src/Iedora.Api/Identity/Common/AuthErrors.cs) (no exceptions/null), mapped to RFC 9457 `ProblemDetails` by the reusable [`Framework.Web.ProblemResults`](framework/Web/Framework.Web/Results/ProblemResults.cs) — the error `code` rides in the problem body as a machine-readable discriminator.
